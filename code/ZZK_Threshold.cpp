...
{
    uint8_t cam_i;   //存放边界的行
    uint8_t cam_j;   //存放边界的列
};

uint8_t cam_static = 90;//当赛道出界，或者计算的灰度阈值异常时使用
uint8_t Threshold0 = 90;
uint8_t cam_thr_mid = 0, cam_thr_max = 0, cam_thr_min = 0;
uint8_t cam_thr = 90, cam_thr0 = 100;//摄像头的阈值
uint8_t HistoGram[256]; //在进行大津的计算阈值时用于统计赛道指定区域的各个阈值的个数数组
int LQ_ary[256];

uint8_t cam_strong_light = 0;//强光标志位
uint8_t cam_normal_light = 0;//正常光标志位
uint8_t cam_wesk_light = 0;  //弱光标志位
int C_W = 159;
...
//*           还有两个宏定义 LQOSTU_MIN02，LQOSTU_MAX02 ，他们分别表示要计算阈值的区域的大小
//* 函数返回：阈值大小 cam_thr
//* 修改时间：2018年3月27日
//*           第一次修改
//* 备 注：
